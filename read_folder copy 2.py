...
class FolderTypeError(DataProcessingError):
    """Exception raised when the folder type is invalid."""
    pass


class RoundNotFoundError(DataProcessingError):
    """Exception raised when a round folder is not found."""
    pass


class CSVProcessingError(DataProcessingError):
    """Exception raised when there is an error processing a CSV file."""
    pass
...
class MappingError(DataProcessingError):
    """Exception raised when there is an error mapping data to a model."""
    pass
    ...
    try:
        for category in folder_categories:
            if category in expected_categories:
                ...
                raise FolderTypeError(f"Invalid folder type: {category}")
        return result_categories
    except FolderTypeError as e:
        logging.error(str(e))
        return None
...
def check_folder_round(root_dir, category_name):
    ...
    try:
        ...
        if not result_round:
            raise RoundNotFoundError(f"No round folders found in category {category_name}")
        return result_round
...
def read_and_process_csv(file_path):
    try:
        ...
        raise CSVProcessingError(f"Error processing CSV file {file_path}: {str(e)}")
...
def insert_model_data(session, df, table_mapper):
    try:
        ...
        raise MappingError(f"Error mapping data to model {table_mapper.__name__}: {e}")
...
def insert_id_master_data(session, df, table_name):
    try:
        ...
        raise MappingError(f"Error mapping data to IdMaster for table {table_name}: {e}")
...
def process_door_data(merged_df, table_mappers, session):
    try:
        ...
        raise DataProcessingError(f"Error processing door data: {e}")
...
def process_under_body_data(merged_df, table_mappers, session):
    try:
        ...
        raise DataProcessingError(f"Error processing under body data: {e}")
...
def process_sm_data(merged_df, table_mappers, session):
    try:
        ...
        raise DataProcessingError(f"Error processing SM data: {e}")
...
def map_to_id_master(row, table_name='under_body_page_data3'):
    try:
        ...
        raise MappingError(f"Error mapping row to IdMaster for table {table_name}: {e}")
...
def get_round_info_for_all_times(merged_df):
    ...
    try:
        ...
        raise DataProcessingError(f"Error retrieving round information: {e}")
...
def main():
    ...
    try:
        ...
        logging.error(f"A data processing error occurred: {e}")
